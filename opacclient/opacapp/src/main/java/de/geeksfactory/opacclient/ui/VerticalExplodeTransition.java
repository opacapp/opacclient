...
/*
 * Copyright (C) 2014 The Android Open Source Project, modified 2015 by Johan v. Forstner
 *
 ...
 * moves views in or out from the edges of the scene. Visibility is determined by both the {@link
 * View#setVisibility(int)} state of the view as well as whether it is parented in the current view
 * hierarchy. Disappearing Views are limited as described in {@link Visibility#onDisappear
...
public class VerticalExplodeTransition extends Visibility {
    ...
    public VerticalExplodeTransition() {
        setPropagation(new CircularPropagation());
    }

    public VerticalExplodeTransition(Context context, AttributeSet attrs) {
        super(context, attrs);
        setPropagation(new CircularPropagation());
    }
    ...
    private void calculateOut(View sceneRoot, Rect bounds, int[] outVector) {
        ...
        double vectorSize = Math.abs(yVector);
        yVector /= vectorSize;
        double maxDistance =
                calculateMaxDistance(sceneRoot, focalY - sceneRootY);
        outVector[0] = 0;
        outVector[1] = (int) Math.round(maxDistance * yVector);
    }

    private static double calculateMaxDistance(View sceneRoot, int focalY) {
        return Math.max(focalY, sceneRoot.getHeight() - focalY);
    }
}
