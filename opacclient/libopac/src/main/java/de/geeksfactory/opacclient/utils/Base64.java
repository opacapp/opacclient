 ...
 * href="http://iharder.net/base64">http://iharder.net/base64</a>.</p> <p/> <p>Example:</p> <p/>
 * <code>String encoded = Base64.encode( myByteArray );</code> <br /> <code>byte[] myByteArray =
 * Base64.decode( encoded );</code> <p/> <p>The <tt>options</tt> parameter, which appears in a few
 * places, is used to pass several pieces of information to the encoder. In the "higher level"
 * methods such as encodeBytes( bytes, options ) the options parameter can be used to indicate such
 * things as first gzipping the bytes before encoding them, not inserting linefeeds, and encoding
 * using the URL-safe and Ordered dialects.</p> <p/> <p>Note, according to <a
 ...
 * although earlier versions broke lines by default.</p> <p/> <p>The constants defined in Base64 can
 * be OR-ed together to combine options, so you might make a call like this:</p> <p/> <code>String
 * encoded = Base64.encodeBytes( mybytes, Base64.GZIP | Base64.DO_BREAK_LINES );</code> <p>to
 * compress the data before encoding it and then making the output have newline characters.</p>
 * <p>Also...</p> <code>String encoded = Base64.encodeBytes( crazyString.getBytes() );</code> <p/>
 * <p/> <p/> <p> Change Log: </p> <ul> <li>v2.3.7 - Fixed subtle bug when base 64 input stream
 * contained the value 01111111, which is an invalid base 64 character but should not throw an
 ...
 * Special thanks to Jim Kellerman at <a href="http://www.powerset.com/">http://www.powerset
 * .com/</a>
 * for contributing the new Base64 dialects. </li> <p/> <li>v2.1 - Cleaned up javadoc comments and
 * unused variables and methods. Added some convenience methods for reading and writing to and from
 * files.</li> <li>v2.0.2 - Now specifies UTF-8 encoding in places where the code fails on systems
 * with other encodings (like EBCDIC).</li> <li>v2.0.1 - Fixed an error when decoding a single byte,
 * that is, when the encoded data was a single byte.</li> <li>v2.0 - I got rid of methods that used
 * booleans to set options. Now everything is more consolidated and cleaner. The code now detects
 * when data that's being decoded is gzip-compressed and will decompress it automatically. Generally
 * things are cleaner. You'll probably have to change some method calls that you were making to
 * support the new options format (<tt>int</tt>s that you "OR" together).</li> <li>v1.5.1 - Fixed
 * bug when decompressing and decoding to a byte[] using <tt>decode( String s, boolean
 * gzipCompressed )</tt>. Added the ability to "suspend" encoding in the Output Stream so you can
 * turn on and off the encoding if you need to embed base64 data in an otherwise "normal" stream
 * (like an XML file).</li> <li>v1.5 - Output stream pases on flush() command but doesn't do
 * anything itself. This helps when using GZIP streams. Added the ability to GZip-compress objects
 * before encoding them.</li> <li>v1.4 - Added helper methods to read/write files.</li> <li>v1.3.6 -
 * Fixed OutputStream.flush() so that 'position' is reset.</li> <li>v1.3.5 - Added flag to turn on
 * and off line breaks. Fixed bug in input stream where last buffer being read, if not completely
 * full, was not returned.</li> <li>v1.3.4 - Fixed when "improperly padded stream" error was thrown
 * at the wrong time.</li> <li>v1.3.3 - Fixed I/O streams which were totally messed up.</li> </ul>
 * <p/> <p> I am placing this code in the Public Domain. Do with it as you will. This software comes
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
    ...
    /**
     * Serializes an object and returns the Base64-encoded version of that serialized object. <p/>
     ...
     * <code>encodeBytes( myData, Base64.GZIP | Base64.DO_BREAK_LINES )</code> <p/> <p/> <p>As of v
    ...
    /**
     * Encodes a byte array into Base64 notation. Does not GZip-compress data. <p/> <p>As of v 2.3,
...
//        }
    ...
    /**
     * Convenience method for encoding data to a file. <p/> <p>As of v 2.3, if there is a error, the
    ...
    /**
     * Convenience method for decoding data to a file. <p/> <p>As of v 2.3, if there is a error, the
     * method will throw an java.io.IOException. <b>This is new to v2.3!</b> In earlier versions, it
     * just returned false, but in retrospect that's a pretty poor way to handle it.</p>
     *
    ...
    /**
     * Convenience method for reading a base64-encoded file and decoding it. <p/> <p>As of v 2.3, if
    ...
    /**
     * Convenience method for reading a binary file and base64-encoding it. <p/> <p>As of v 2.3, if
     * there is a error, the method will throw an java.io.IOException. <b>This is new to v2.3!</b>
     * In earlier versions, it just returned false, but in retrospect that's a pretty poor way to
     * handle it.</p>
     *
