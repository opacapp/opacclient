	        ...
	        for (int i = 0; i < resultsJsonArray.length(); i++) {
        		...
        		if(contains(result.getJSONArray("types"), "locality")) {
        			...
        			for (int j = 0; j < addressComponents.length(); j++) {
        				...
        				if(contains(component.getJSONArray("types"), "locality")) {
        					city.name = component.getString("long_name");
        				} else if (contains(component.getJSONArray("types"), "administrative_area_level_1")) {
        					city.state = component.getString("long_name");
        				} else if (contains(component.getJSONArray("types"), "country")) {
        					city.country = component.getString("long_name");
        				}
        			}	
        			city.lat = result.getJSONObject("geometry").getJSONObject("location").getDouble("lat");
        			city.lon = result.getJSONObject("geometry").getJSONObject("location").getDouble("lng");
        			resultList.add(city);
        		}
	        }
	    } catch (JSONException e) {
